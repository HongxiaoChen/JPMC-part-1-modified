...
def plot_figure5(samples_hnn, samples_lhnn):
    """
    generate figure 5: compare the q1 trajectory of hnn and lhnn
    
    Parameters
    ----------
    samples_hnn : np.ndarray
        the samples generated by hnn
...
def run_figure5():
    """run the experiment and generate figure 5"""
    # set the basic parameters
    args = get_args()
    args.dist_name = 'nD_Rosenbrock'
    args.input_dim = 6  # 3D Rosenbrock
    args.total_samples = 50  # use more samples, consistent with the paper
    args.burn_in = 0  # need to burn-in
    ...
    samples_hnn = process_samples(samples_hnn)
    ...
    # generate the figure
    plot_figure5(samples_hnn.numpy(), samples_lhnn.numpy())

    # print the statistics
    print("\nComparison statistics:")
    # extract the relevant information from kernel_results
    print(f"HNN acceptance rate: {tf.reduce_mean(tf.cast(kernel_results_hnn.is_accepted, tf.float32)):.4f}")
    print(f"L-HNN acceptance rate: {tf.reduce_mean(tf.cast(kernel_results_lhnn.is_accepted, tf.float32)):.4f}")
